...
async function auth_post(req, res) {
    ...
    if (new_email && new_password && name && dob) {
        try {
            ...
            console.log(err)
            res.render("auth", { exist: true,url:req.myurl })
        }
    }
        ...
        if (!data) {
            res.render("auth", { no_email: true,url:req.myurl })
        }
        else if (data.Password !== password) {
            res.render("auth", { no_pass: true,url:req.myurl })
        }
        else {
            const token = set_user(new_data)
            const currentDateUTC = new Date();
        ...
        res.render("auth", { missing: true,url:req.myurl })
    }
...
async function auth_post_rest(req, res) {
    ...
    console.log(email,password,new_email,new_password,name,dob)
